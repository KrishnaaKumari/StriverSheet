...
public class TopView {
	...
	public static ArrayList printTopViewTree(TreeNode root)
	{
		if(root==null)
			return null;
		...
		while(!pending.isEmpty())
		{
			...
			if(p.root.left!=null)
				pending.add(new Pair(p.root.left,p.hd-1));
			if(p.root.right!=null)
				pending.add(new Pair(p.root.right,p.hd+1));
		}
	...
	public static void main(String[] args) {
		...
		ArrayList<Integer> ans = printTopViewTree(root);
		for(Integer i : ans)
			System.out.print(i+" ");
		System.out.println();
	}
